...
\end{align*}
%\end{minipage}
这里也同样略去了积分函数和时间等参量。考虑到该后向传播过程需要知道函数的输入，我们需要用一种方式反向遍历状态，最直接的方式是将所有中间状态缓存。此时前向计算（左侧）和后向梯度传播（右侧）可表达为
...
\begin{align*}
    ...
    &s_{i+1} = {\rm ODEStep}(s_{i})  ~\# ~\text{重计算}\\
...
\begin{align*}
    &\text{\# 正向计算}\\
    &s_{i+1} \leftarrow 0\\
    &s_{i+1} \mathrel{+}= {\rm ODEStep}(s_{i})\\
    &s_{i+2} \leftarrow 0\\
    &s_{i+2} \mathrel{+}= {\rm ODEStep}(s_{i+1})\\
    &s_{i+1} \mathrel{-}= {\rm ODEStep}(s_{i}) ~\#~\text{反计算}\\
...
\begin{align*}
    &\text{\# 反向计算}\\
    &s_{i+1} \leftarrow 0\\
    &s_{i+1} \mathrel{+}= {\rm ODEStep}(s_{i}) ~\# ~\text{计算}~ s_{i+1}\\
    &s_{i+2} \mathrel{-}= {\rm ODEStep}(s_{i+1})\\
    &s_{i+2} \rightarrow 0\\
    &s_{i+1} \mathrel{-}= {\rm ODEStep}(s_{i})\\
    &s_{i+1} \rightarrow 0
