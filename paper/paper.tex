...
\subsection{洛伦兹方程求解}
洛伦兹系统是人们研究混沌的经典模型，它描述了一个三维空间中的一个点的坐标在$(x,y,z)$的动力学由洛伦兹方程决定
\begin{align*}
    \frac{\D x}{\D t} &= \sigma(y - x),\\
    \frac{\D y}{\D t} &= x(\rho -z) - y,\\
    \frac{\D z}{\D t} &= xy-\beta z.
\end{align*}
其中，$\sigma$, $\rho$和$\beta$为三个控制参数。数值模拟中，我们用Runge-Kutta方法对时间部分积分并得到末了位置。
它对控制参数和初始位置的导数决定了末了位置对控制参数和初始位置的敏感度。
