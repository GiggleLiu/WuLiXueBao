...
\end{figure}
数学中，在对一个输入变量$p$求导时，会让它携带一个无穷小量$dp$，并通过对这个无穷小量的运算完成对程序的求导。比如当作用函数$f$时，会有如下链式法则
\begin{align}
    f(\vec x+ \frac{\partial \vec x}{\partial p} dp) = f(\vec x) + \frac{\partial f(\vec x)}{\partial \vec x}\frac{\partial \vec x}{\partial p} dp
\end{align}
其中$\frac{\partial f(\vec x)}{\partial \vec x}$为局域雅可比矩阵，
实际程序实现中，这个局域雅可比矩阵并不需要构造出来。由于任何程序都具有可拆分为基础指令的特点，人们把程序拆解为基础标量指令，并在这些基础指令上实现自动微分的局域变换规则。
具体来说一个标量会同时记录它的数值和一阶小量的系数$(x_i, \frac{\partial x_i}{\partial p})$，人们通过重新定义它的基本运算规则 (如$\sin: (x, \overset{\rightharpoonup} x) \mapsto (\sin x, \overset{\rightharpoonup} x\cos x)$，其中$\overset{\rightharpoonup} x = \frac{\partial x}{\partial p}$) 使得其在计算同时更新一阶小量的系数。
...
\begin{align*}
    (s_{i+1}, \frac{d s_{i+1}}{d s_0}) = ({\rm ODEStep}(s_{i}, t_i, \theta, \Delta t), \frac{d s_{i+1}}{\partial s_i} \frac{\partial s_{i}}{\partial s_0})\\
    (\theta, \frac{d s_{i+1}}{d \theta}) = ({\rm ODEStep}(s_{i}, t_i, \theta, \Delta t), \frac{\partial s_{i+1}}{\partial s_i} \frac{d s_{i}}{d \theta} + \frac{\partial s_{i+1}}{\partial \theta})
\end{align*}
其中第二个域为一阶无穷小量的系数（导数），由于状态$s_i, s_{i+1}$和控制参数$\theta$均可包含多个变量，上述偏微分均解释为雅可比行列式，
真实的计算中，并不需要构造出具体的局域雅可比矩阵$\frac{\partial s_{i+1}}{\partial s_i}$并做复杂度较高的矩阵乘法，而是通过上述对基本运算符的重载来实现。
%因为一些如\texttt{ForwardDiff}这样优秀的前向自动微分库，我们并不需要手动实现这些链式法则以及基础微分规则。

值得注意的是，如果每次只求导单个变量，求导时间会随着需求导的变量的数目线性增长。但如果同时更行整个雅可比行列式，但此时空间随着求导变量数目线性增加。
...
\subsection{后向自动微分}
图~\ref{fig:autodifftypes} (b) 展示了后向自动微分的前向计算过程和梯度反向传播过程，它通过如下链式法则完成求导
