...
@testset "detector" begin
    c0 = 3300^2*(ones(203, 203))
    detector_locs, target_pulses = targetpulses_three_layer()
    res1, g1 = getgrad_three_layer(method=:bennett, usecuda=false, target_pulses=target_pulses, detector_locs=detector_locs, c0=copy(c0))
    res2, g2 = getgrad_three_layer(method=:bennett, usecuda=true, target_pulses=target_pulses, detector_locs=detector_locs, c0=copy(c0))
    @test g2[1] isa CuArray
    @test g1[1] ≈ Array(g2[1])
    @test g1[2] ≈ Array(g2[2])
    @test g1[3] ≈ Array(g2[3])
