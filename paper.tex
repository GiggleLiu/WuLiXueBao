...
\subsection{鹅卵石游戏}
鹅卵石游戏是在一维格子上的单人游戏，它最初被提出描述可逆计算中的时间与空间的权衡。游戏开始，玩家拥有一堆鹅卵石以及一个一维排布的$G$个格子，标记为$0,1,2\ldots G$，并且在$0$号格子上有一个预先布置的鹅卵石。游戏目标是从自己的堆中取尽可能最少的鹅卵石，或是使用尽可能少的步骤数触发游戏结束。
其规则为
...
鹅卵石游戏最省步骤数的玩法，也是最消耗空间的玩法是用$G$个鹅卵石依次铺至终点格子$G$，此时时间复杂度和空间复杂度均为$\bigO(T)$，此处$T$为格子数。
